...
export const ALL_LEAVES = TYPE_TREE.flatMap((n) => n.children ? n.children.map((c) => c.key) : [n.key])

// raw type → display label (used for scorecard chips / report grouping)
export const RAW_TO_LABEL = TYPE_TREE.reduce((acc, n) => {
  if (n.children) n.children.forEach((c) => { acc[c.key] = n.label })
  else acc[n.key] = n.label
  return acc
}, {})
