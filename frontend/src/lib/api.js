// Central API client. Every backend endpoint is wired here.
// Same-origin in production (Flask serves the built app); dev uses Vite proxy.
...
export const api = {
  ...
  // ── SSE stream URLs (used by useStream hook) ──
  fetchUrl: (rid, params = {}) => `/api/fetch/${rid}${qs(params)}`,
  matchUrl: (rid, params = {}) => `/api/match/${rid}${qs(params)}`,
  uploadUrl: (rid, params = {}) => `/api/upload/${rid}${qs(params)}`,

  // ── downloads ──
  exportXlsxUrl: (rid) => `/api/export/${rid}`,
  reportXlsxUrl: (rid) => `/api/report/${rid}?format=xlsx`,
